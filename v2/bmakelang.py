...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python bmakelang.py <Bmakefile>")
        sys.exit(1)

    script_path = sys.argv[1]  # Take the file name as an argument
    try:
        bmake = BmakeLang(script_path)  # Parse the file passed as argument
        target = sys.argv[2] if len(sys.argv) > 2 else "all"  # Default to "all" target
